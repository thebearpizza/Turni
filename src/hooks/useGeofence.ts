...
export function useGeofence() {
  ...
  function check(restaurantLat?: number, restaurantLng?: number): Promise<GeofenceStatus> {
    // Fail-closed: no geolocation support → block
    ...
    return new Promise(resolve => {
      navigator.geolocation.getCurrentPosition(
        ...
        _err => {
          // Fail-closed: denied, timeout, or unavailable all block the scan
          setStatus('denied')
          resolve('denied')
        },
